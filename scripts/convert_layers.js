...
const LAYERS_DIR = './content/layers';
const PROPS_IMPORT = "import { Project, Type, LiveStatus, RiskFactor, EntityType, Site, RiskSection, ContentSection, RiskCategory } from '../props';\n\n";
...
function convertObjectKeys(obj) {
  ...
  Object.entries(obj).forEach(([key, value]) => {
    ...
    if (key === 'layerType') {
      ...
      result[newKey] = value === true || value === 'yes' || value === 'true';
    } else if (key === 'links') {
      ...
      result[newKey] = value.map(factor => factor ? `RiskFactor.${getRiskFactor(factor)}` : '');
