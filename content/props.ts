...
export enum TokenSnippet { //TODO: Janusz to add more here
    ...
    SparkBTC = "To deposit funds into Spark, users generate an aggregate public key with the statechain entity. Users construct a deposit transaction to an address controlled by this public key and then co-constructs two pre-signed transactions with the statechain entity.\n\nOne transaction contains a unilateral exit path for the user. The other implements a decrementing timelock which is tweaked after ownership of an offchain UTXO is transferred.\n\nIn statechain transfers, relative decrementing timelocks ensure that users can execute an exit transaction prior to a previous owner (if they previous owner broadcasts their own exit transaction maliciously).",
