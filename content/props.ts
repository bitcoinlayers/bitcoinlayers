...
export enum TokenSnippet { //TODO: Janusz to add more here
    ...
    SparkBTC = "Users custody funds collectively with the statechain entity. When depositing into Spark, users co-construct a pre-signed transactions with the statechain entity. This pre-signed transaction contains a unilateral exit path for the user if the statechain entity becomes unresponsive.\n\nWhen receiving funds via a statechain transfer, decrementing timelocks ensure that users can execute an exit transaction prior to a previous owner (if they previous owner broadcasts their own exit transaction maliciously).\n\nUsers additionally trust the statechain entity to delete the keyshare it held with the previous owner so it cannot collectively spend funds with past owners of the UTXO.",
...
export enum BitcoinSecuritySnippet { //TODO: Janusz to add more here
    ...
    OffchainUTXO = "The protocol enables users to unilaterally exit, thus only relying on bitcoin security for custody of funds.",
