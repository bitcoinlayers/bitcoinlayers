...
class BaseAnalyzer(ABC):
    ...
    def save_analysis_report(self, analysis_data: Dict[str, Any], 
        ...
        # Create wrapper/network-specific directory structure if layer_slug is provided
        if layer_slug:
            wrapper_dir = os.path.join(output_dir, layer_slug)
            # For Bitcoin analyzers, add network subdirectory if network is specified
            if hasattr(self, 'network') and self.network and self.network.lower() != 'mainnet':
                network_slug = self.network.lower().replace(" ", "_").replace("-", "_")
                final_output_dir = os.path.join(wrapper_dir, network_slug)
            else:
                # For Bitcoin mainnet, use 'bitcoin' as the network slug
                final_output_dir = os.path.join(wrapper_dir, 'bitcoin')
