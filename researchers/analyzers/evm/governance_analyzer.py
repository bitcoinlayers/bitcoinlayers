...
TARGET_CONTRACT = "0xE3d981643b806FB8030CDB677D6E60892E547EdA"  # Bridge/infrastructure contract to analyze
...
CONTRACT_ROLE = "L1 Messenger"  # Role of the target contract (e.g., "Bridge", "Multisig", "Timelock", "Governance")
...
class GovernanceAnalyzer:
    ...
    def check_proxy_pattern(self, address: str) -> Dict[str, Any]:
        """
        Check if the contract follows EIP-1967 or ResolvedDelegateProxy pattern
        """
        proxy_info = {
            "is_proxy": False,
        ...
        try:
            # First, check for EIP-1967 proxy pattern
            ...
            # If not EIP-1967, check for ResolvedDelegateProxy pattern
            if not proxy_info["is_proxy"]:
                resolved_proxy_info = self.check_resolved_delegate_proxy(address)
                if resolved_proxy_info["is_proxy"]:
                    proxy_info.update(resolved_proxy_info)
    ...
    def check_resolved_delegate_proxy(self, address: str) -> Dict[str, Any]:
        """
        Check if the contract follows ResolvedDelegateProxy pattern
        Used by Optimism and similar systems
        """
        proxy_info = {
            "is_proxy": False,
            "implementation_address": None,
        ...
        try:
            # Get contract ABI to check for ResolvedDelegateProxy pattern
            abi_data = self.get_contract_abi(address)
            if not abi_data or not abi_data.get('abi'):
                return proxy_info
                
            contract_abi = abi_data['abi']
            
            # Check if this looks like a ResolvedDelegateProxy based on ABI structure
            ...
            # ResolvedDelegateProxy pattern: constructor + fallback + minimal/no functions
            if (constructor_inputs and has_fallback and function_count <= 2):
                # Check constructor parameters for AddressManager and implementation name
                has_address_manager = False
                has_implementation_name = False
                
                for input_param in constructor_inputs:
                    if 'AddressManager' in input_param.get('internalType', '') or 'addressManager' in input_param.get('name', ''):
                        has_address_manager = True
                    if 'implementationName' in input_param.get('name', '') or 'name' in input_param.get('name', ''):
                        has_implementation_name = True
                
                if has_address_manager and has_implementation_name:
                    proxy_info["is_proxy"] = True
                    proxy_info["proxy_type"] = "ResolvedDelegateProxy"
                    print(f"✅ Detected ResolvedDelegateProxy pattern from constructor")
                    
                    # Try to resolve implementation by analyzing transaction history
                    resolved_info = self.resolve_delegate_proxy_implementation(address)
                    if resolved_info:
                        proxy_info.update(resolved_info)
                    
                    return proxy_info
            
            # Fallback: try direct function calls if available
            contract = self.w3.eth.contract(address=address, abi=contract_abi)
            address_manager = None
            proxy_name = None
            
            try:
                if hasattr(contract.functions, 'addressManager'):
                    address_manager = contract.functions.addressManager().call()
                    print(f"✅ Found addressManager: {address_manager}")
            except Exception:
                pass
                
            try:
                if hasattr(contract.functions, 'name'):
                    proxy_name = contract.functions.name().call()
                    print(f"✅ Found proxy name: {proxy_name}")
            except Exception:
                pass
            
            # If we have both addressManager and name, try to resolve implementation
            if address_manager and proxy_name:
                try:
                    # Get AddressManager ABI and resolve the implementation
                    am_abi_data = self.get_contract_abi(address_manager)
                    if am_abi_data and am_abi_data.get('abi'):
                        am_abi = am_abi_data['abi']
                        am_contract = self.w3.eth.contract(address=address_manager, abi=am_abi)
                        
                        # Try getAddress(name) function
                        if hasattr(am_contract.functions, 'getAddress'):
                            implementation = am_contract.functions.getAddress(proxy_name).call()
                            if implementation and implementation != '0x' + '00' * 40:
                                proxy_info["is_proxy"] = True
                                proxy_info["proxy_type"] = "ResolvedDelegateProxy"
                                proxy_info["implementation_address"] = Web3.to_checksum_address(implementation)
                                proxy_info["address_manager"] = Web3.to_checksum_address(address_manager)
                                proxy_info["name"] = proxy_name
                                print(f"✅ Resolved implementation: {implementation}")
                        
                        # Try addresses(name) function as fallback
                        elif hasattr(am_contract.functions, 'addresses'):
                            implementation = am_contract.functions.addresses(proxy_name).call()
                            if implementation and implementation != '0x' + '00' * 40:
                                proxy_info["is_proxy"] = True
                                proxy_info["proxy_type"] = "ResolvedDelegateProxy"
                                proxy_info["implementation_address"] = Web3.to_checksum_address(implementation)
                                proxy_info["address_manager"] = Web3.to_checksum_address(address_manager)
                                proxy_info["name"] = proxy_name
                                print(f"✅ Resolved implementation via addresses(): {implementation}")
                
                except Exception as e:
                    print(f"🔍 Could not resolve implementation: {e}")
            
            # Even if we can't resolve the implementation, we can still detect it's a ResolvedDelegateProxy
            elif address_manager or proxy_name:
                proxy_info["is_proxy"] = True
                proxy_info["proxy_type"] = "ResolvedDelegateProxy"
                proxy_info["address_manager"] = address_manager
                proxy_info["name"] = proxy_name
                print(f"✅ Detected ResolvedDelegateProxy (partial info)")
    ...
    def resolve_delegate_proxy_implementation(self, address: str) -> Dict[str, Any]:
        """
        Try to resolve ResolvedDelegateProxy implementation by examining deployment transaction
        """
        resolved_info = {}
        
        try:
            # Get the contract creation transaction to extract constructor parameters
            print(f"🔍 Resolving ResolvedDelegateProxy implementation for {address}")
            
            # This is a simplified approach - in practice, you might need to:
            # 1. Get the creation transaction
            # 2. Decode the constructor parameters
            # 3. Extract AddressManager and implementationName
            # 4. Call AddressManager.getAddress(implementationName)
            
            # For now, try some common AddressManager addresses and implementation names
            common_address_managers = [
                "0xdE1FCfB0851916CA5101820A69b13a4E276bd81F",  # Optimism AddressManager
                "0x8EfB6B5c4767B09Dc9AA6Af4eAA89F749522BaE2",  # Common L1 AddressManager
            ]
            ...
            for am_address in common_address_managers:
                try:
                    am_abi_data = self.get_contract_abi(am_address)
                    if am_abi_data and am_abi_data.get('abi'):
                        am_contract = self.w3.eth.contract(address=am_address, abi=am_abi_data['abi'])
                        
                        for impl_name in common_implementation_names:
                            try:
                                if hasattr(am_contract.functions, 'getAddress'):
                                    implementation = am_contract.functions.getAddress(impl_name).call()
                                elif hasattr(am_contract.functions, 'addresses'):
                                    implementation = am_contract.functions.addresses(impl_name).call()
                                else:
                                    continue
                                    
                                if implementation and implementation != '0x' + '00' * 40:
                                    resolved_info["implementation_address"] = Web3.to_checksum_address(implementation)
                                    resolved_info["address_manager"] = Web3.to_checksum_address(am_address)
                                    resolved_info["name"] = impl_name
                                    print(f"✅ Resolved implementation: {implementation} via {impl_name}")
            ...
            print(f"⚠️ Could not resolve implementation for ResolvedDelegateProxy")
            
        except Exception as e:
            print(f"Error resolving ResolvedDelegateProxy: {e}")
    ...
    def generate_markdown_report(self, address: str, analysis_data: dict):
        ...
        # Create wrapper/network-specific directory
        if WRAPPER_TOKEN_NAME and LAYER_NAME:
            wrapper_slug = WRAPPER_TOKEN_NAME.lower().replace(" ", "_").replace("-", "_")
            # Use TARGET_ANALYSIS_NETWORK if defined, otherwise use NETWORK
            target_network = globals().get('TARGET_ANALYSIS_NETWORK', NETWORK)
            network_slug = target_network.lower().replace(" ", "_").replace("-", "_")
            output_dir = os.path.join("analysis-reports", wrapper_slug, network_slug)
        elif LAYER_NAME:
...
| Implementation | {'[' + proxy_info['implementation_address'] + '](' + (EXPLORER_BASE_URL if 'EXPLORER_BASE_URL' in globals() else 'https://etherscan.io/address/') + proxy_info['implementation_address'] + ')' if proxy_info['is_proxy'] and proxy_info.get('implementation_address') else 'Not Available' if proxy_info['is_proxy'] else 'N/A'} |
...
"""
            if proxy_info.get('implementation_address'):
                markdown_content += f"- **Implementation:** [{proxy_info['implementation_address']}]({EXPLORER_BASE_URL if 'EXPLORER_BASE_URL' in globals() else 'https://etherscan.io/address/'}{proxy_info['implementation_address']})\n"
            
            proxy_type = proxy_info.get('proxy_type', 'Unknown')
            markdown_content += f"- **Proxy Pattern:** {proxy_type}\n"
            
            if proxy_info.get('address_manager'):
                markdown_content += f"- **Address Manager:** [{proxy_info['address_manager']}]({EXPLORER_BASE_URL if 'EXPLORER_BASE_URL' in globals() else 'https://etherscan.io/address/'}{proxy_info['address_manager']})\n"
            
            if proxy_info.get('name'):
                markdown_content += f"- **Proxy Name:** {proxy_info['name']}\n"
            
            if proxy_info.get('admin_address'):
                markdown_content += f"- **Admin:** [{proxy_info['admin_address']}]({EXPLORER_BASE_URL if 'EXPLORER_BASE_URL' in globals() else 'https://etherscan.io/address/'}{proxy_info['admin_address']})\n"
...
"""
    ...
    def generate_json_report(self, bridge_address: str, token_address: str, analysis_data: dict):
        ...
        # Create wrapper/network-specific directory for token file
        if WRAPPER_TOKEN_NAME and LAYER_NAME:
            wrapper_slug = WRAPPER_TOKEN_NAME.lower().replace(" ", "_").replace("-", "_")
            # Use TARGET_ANALYSIS_NETWORK if defined, otherwise use NETWORK
            target_network = globals().get('TARGET_ANALYSIS_NETWORK', NETWORK)
