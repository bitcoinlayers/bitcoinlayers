...
export default function AggregatedTVLChart({
    ...
    const totalBTC = useMemo(() => {
        ...
        const latestDate = data.reduce((latest, current) => 
            latest && current.date && latest > current.date ? latest : current.date, data[0].date);
        return data
            .filter(item => item.date === latestDate)
            .reduce((sum, item) => sum + (item.amount || 0), 0);
    }, [data]);

    const btcPriceData = pricesData?.find(price => price.token_slug === "btc");

    const tokens = useMemo(() => 
        chartType === "combined" 
            ? ["BTC"] 
            : [...new Set(data?.map(item => item[itemNameKey]).filter(Boolean))],
    ...
    const processedData = useMemo(() => {
        ...
        return data.reduce((acc, item) => {
            const key = chartType === "combined" ? "BTC" : item[itemNameKey] || "unknown";
            const existingEntry = acc.find(entry => entry.date === item.date);
            
            if (existingEntry) {
                existingEntry[key] = (existingEntry[key] || 0) + (item.amount || 0);
            } else {
    ...
    const filterDataByDateRange = useCallback((data: any[]) => {
        const now = new Date();
        const monthsToSubtract = { "1y": 12, "1mo": 1, "3mo": 3 }[dateRange] || 3;
        ...
        <Card className="bg-background space-y-4">
            <CardHeader className="flex flex-col items-stretch space-y-0 border-b p-0 sm:flex-row">
                <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6">
                    <CardTitle className="flex font-semibold items-center gap-2">{title}</CardTitle>
                    <CardDescription className="text-xs flex flex-wrap">{description}</CardDescription>
                </div>
                <div className="flex">
                    {/* TODO: Add a button to toggle the chart type */}
                    <div className="relative z-30 flex flex-1 flex-col justify-center gap-1 border-t px-6 py-4 text-left even:border-l sm:border-l sm:border-t-0 sm:px-8 sm:py-6" data-active={false} onClick={() => {}}>
                        <span className="text-xs text-muted-foreground">TVL</span>
                        <span className="font-bold">{totalBTC.toFixed(2)} BTC</span>
            ...
            <CardContent>
                <ChartContainer config={chartConfig} className={cn("w-full watermark", chartHeight)}>
                    <AreaChart
                    ...
                    >
                        ...
                        <ChartTooltip
                            ...
                            content={
                                <ChartTooltipContent
                                    labelFormatter={value => formatDate(value, { month: "short", day: "numeric", year: "numeric" })}
                                    className="w-60"
                                    sort="desc"
                                />
                            }
                        />
                        {tokens?.sort().map(item => (
                            <Area
                                key={item}
                                name={item}
                                dataKey={item!}
                        ...
                        {showLegend && <ChartLegend content={<ChartLegendContent className="flex flex-wrap" />} />}
                    </AreaChart>
                </ChartContainer>
                <div className="flex mt-6">
                    {showDivisionButtons && (
                        <ToggleGroup
                            type="single"
                            value={chartType}
                            onValueChange={value => value && value !== chartType && setChartType(value)}
                            className="!space-x-0"
                        >
                            {["combined", "separate"].map(value => (
                                <ToggleGroupItem key={value} value={value} className="border text-xs" size="sm">
