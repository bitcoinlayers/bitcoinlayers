...
import TokenContractAnalysisDropdown from "@/components/layer/token-contract-analysis-dropdown";
...
// Get contract address for a specific peg on a specific network
// Only includes networks where we have actual token contract analysis data
const getContractAddress = (pegSlug: string, chainSlug: string): string | null => {
    const contractMapping: { [key: string]: string } = {
        // Lombard LBTC contracts - only networks with analysis data available
        'lombard-lbtc:ethereum': '0x8236a87084f8b84306f72007f36f2618a5634494',
        'lombard-lbtc:base': '0xecac9c5f704e954931349da37f60e39f515c11c1',  
        'lombard-lbtc:bob': '0xa45d4121b3d47719ff57a947a9d961539ba33204',
        // Additional peg-chain combinations can be added as analysis data becomes available
    };
    
    const key = `${pegSlug}:${chainSlug}`;
    return contractMapping[key] || null;
};
    ...
    return (
        <Card className="w-full bg-card">
            ...
            <CardContent className="space-y-4">
                ...
                {/* Taproot Analysis - Show for pegs that have taproot analysis available */}
                {/* Currently enabled for Lombard LBTC and Babylon Staked BTC */}
                {(implementation.pegSlug === "lombard-lbtc" || implementation.pegSlug === "babylonstaked-btc") && (
                    <div className="w-full mt-6">
                        <TaprootAnalysisSection 
                            infrastructureSlug={implementation.pegSlug} 
                            autoExpand={true}
                        />
                    </div>
                )}

                {/* Token Contract Analysis - Show for pegs with contract addresses on specific networks */}
                {(() => {
                    const contractAddress = getContractAddress(implementation.pegSlug, implementation.chainSlug);
                        ...
                        <div className="w-full mt-6">
                            <div className="space-y-4">
                                ...
                                <TokenContractAnalysisDropdown 
                                    ...
                                    wrapperName={implementation.pegName}
                                    networkName={implementation.chainName}
                                    autoExpand={true}
                                />
                            </div>
                        </div>
