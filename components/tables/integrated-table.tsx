...
const IntegratedTable = ({ data, headers }: Props) => {
    ...
    const sortAndFilterData = useMemo(() => {
        // First filter to only integrated projects
        const integratedOnly = data.filter(item => item.entityCategory === EntityCategory.Integrated);
        
        const sorted = [...integratedOnly].sort((a, b) => {
            let valueA, valueB;
            switch (sortBy) {
                case "Name":
