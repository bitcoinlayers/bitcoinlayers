...
const OtherTable = ({ data, headers, hideHeader = false, hideCard = false, pegSupplyView = "pegs", onPegSupplyViewChange }: Props) => {
    ...
    const sortAndFilterData = useMemo(() => {
        // First filter to only other projects
        const otherOnly = data.filter(item => item.entityCategory === EntityCategory.Other);
        
        const sorted = [...otherOnly].sort((a, b) => {
            let valueA, valueB;
            switch (sortBy) {
                case "Name":
                ...
                <CardHeader className="flex flex-col items-stretch space-y-0 border-b p-0 sm:flex-row border-none">
                    <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6">
                        <CardTitle className="flex">
                            <LayersIcon className="mr-3" /> Other
                        </CardTitle>
                        <CardDescription>
                            Other bitcoin-related networks and projects.
                        </CardDescription>
                    </div>
                </CardHeader>
            )}
